...
- 表单校验自愈
  ...
  - 提交后必须命中成功信号（弹窗关闭、URL/页面状态变化、成功提示、新记录、校验消失）之一，否则进入“状态不确认”分支并判定失败，避免把 click 成功误判为业务成功。
- 表单感知编排
...
| `click` | 执行点击，支持重试、跳转检测、提交类校验自愈与提交后成功信号确认 |
...
| 表单处理 | 常规失败即报错 | 支持提交前预检、校验自愈和提交后状态确认 |
