...
const getElementSnapshot = async (locator) => {
...
const isLikelyCssSelector = (target) => {
    return /^[\s]*[#.\[]/.test(target);
};
const getInspectFallbackLocator = async (runId, target) => {
    ...
    for (let i = 0; i < total; i += 1) {
        ...
        const searchable = `${snapshot.text} ${snapshot.ariaLabel} ${snapshot.placeholder} ${snapshot.idAttr} ${snapshot.nameAttr}`.toLowerCase();
...
const buildCandidates = async (runId, target, maxCandidates) => {
    const page = await browserManager.getPage(runId);
    const strategyDefs = [];
    if (isLikelyCssSelector(target)) {
        strategyDefs.push({ name: "stableSelector", score: 120, locator: page.locator(target) });
    }
    strategyDefs.push({ name: "text", score: 100, locator: page.getByText(target, { exact: false }) }, { name: "label", score: 90, locator: page.getByLabel(target) }, { name: "placeholder", score: 80, locator: page.getByPlaceholder(target) }, { name: "buttonRole", score: 70, locator: page.getByRole("button", { name: target }) });
    const candidates = [];
    const dedupe = new Set();
    const collectFromStrategy = async (strategy) => {
        let count = 0;
        try {
...
export const registerFindTool = (server) => {
    const definition = {
        description: "定位页面元素",
        parameters: z.object({
        ...
        execute: async ({ run_id, target, return_candidates, max_candidates, retry_count, }) => {
            let candidates = [];
