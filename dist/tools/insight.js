...
const getSnapshot = async (maxTextLen, locator) => {
    return locator.evaluate((el, limit) => {
        const normalize = (v) => v.trim().replace(/\s+/g, " ").slice(0, limit);
        const element = el;
        return {
            ...
            typeAttr: normalize(element.type ?? ""),
        };
    }, maxTextLen);
};
const collectPageElements = async ({ runId, includeHidden, query, maxTextLen, }) => {
...
export const registerInsightTools = (server) => {
    const inspectSummaryDefinition = {
        ...
        execute: async ({ run_id, max_elements, offset, include_hidden, query, compact, max_text_len, }) => {
            ...
            const tagSummary = elements.reduce((acc, item) => {
                const key = String(item.tag ?? "unknown");
                acc[key] = (acc[key] ?? 0) + 1;
                return acc;
            }, {});
            const result = {
    ...
    const inspectDetailDefinition = {
        ...
        execute: async ({ run_id, element_ids, compact, }) => {
